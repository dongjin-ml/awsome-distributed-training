...
def benchmark_dataloader(dataset, batch_size, num_workers, pin_memory, prefetch_factor, num_epochs, 
                         use_fp16=False, use_prefetch=False, gpu_iterations=3):
    """
    ...
    processed_samples = 0
    total_batches = min(len(dataloader), 50)  # 20 배치만 테스트
...
def main():
    ...
    # 테스트할 worker 수 범위
    worker_counts = [2] if torch.cuda.is_available() else [0, 1, 2]
    
    # pin_memory 옵션
    pin_memory_options = [True]
    
    # prefetch 옵션 추가 (CUDA Streaming)
    prefetch_options = [False, True]
    
    # prefetch_factor 옵션 추가 (dataloader의 prefetch_factor)
    prefetch_factor_options = [2]
                ...
                for use_cache in cache_options:
                    ...
                    if num_workers == 0:
                        ...
                        if is_main_process():
                            ...
                            main_process_print(f'custom_prefetch={use_prefetch}')
                            main_process_print(f'use_cache={use_cache}')

                            # main_process_print(
                            #     f"\n테스트 설정: workers={num_workers}, pin_memory={pin_memory}, custom_prefetch={use_prefetch}, use_cache={use_cache}"
                            #     )
